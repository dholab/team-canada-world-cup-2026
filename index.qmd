...
The underlying dataset — the complete record of collected samples (739 rows:
182 respiratory cartridges × 4 targets, plus 11 cartridges that produced no
valid test data) — and the script that builds every figure are in the
repository:
...
`status` flags each cartridge as `valid`, `spc_negative`, `probe_error`, or
`no_valid_data`. The figures show valid runs only, so samples with a negative
control, a probe error, or no valid data appear in this table but not in the
figures.
