...
**Figure 3. Human viruses detected by metagenomic sequencing of the Houston air
...
present. Columns are the six sequenced samples, labelled by room, sampler,
GeneXpert SARS-CoV-2 status where a same-room result exists, and sampling start
time and elapsed duration. Rows are the human viruses detected, with Merkel cell
polyomavirus and SARS-CoV-2 on top and the rest ordered by total distinct reads.
Each cell reports distinct (deduplicated) reads for that virus in that sample,
with fill mapped to that count on a light-to-deep blue scale. A cell shows 0 when
the virus was sought and not found (SARS-CoV-2 was assessed in every sample) and
is left unshaded when not detected. SARS-CoV-2 was recovered from both meal-room
...
The wastewater comparison in [online supplemental figure 1](#fig-s1-wastewater)
...
::: {.content-visible when-format="pdf"}
![](analysis/figure3_wastewater.png)
:::
